...
from sdp.common.normalize_util import (
    ...
    array_to_stats,
    get_range_symmetric_normalizer_from_stat, to_torch_stats,
    get_torch_range_symmetric_normalizer_from_stat
)

register_codecs()


class RobomimicReplayPointCloudGripperEqualScaleDataset(BaseImageDataset):
    def __init__(self,
                 shape_meta: dict,
                 dataset_path: str,
                 horizon=1,
    ...
    def get_normalizer(self, mode='limits', **kwargs) -> LinearNormalizer:
        ...
        pcd_stat = to_torch_stats(
            self.replay_buffer['point_cloud'][:].reshape(-1, self.replay_buffer['point_cloud'].shape[-1]))
        xyz_h_range = (pcd_stat['max'][:3] - pcd_stat['min'][:3]).max() / 2
        pcd_stat['min'][:3] = pcd_stat['mean'][:3] - xyz_h_range
        pcd_stat['max'][:3] = pcd_stat['mean'][:3] + xyz_h_range
        normalizer['point_cloud'] = get_torch_range_symmetric_normalizer_from_stat(pcd_stat)

        # pcd_data = {
        #     'point_cloud': self.replay_buffer['point_cloud'],
        # }
        # normalizer.fit(data=pcd_data, last_n_dims=1, mode=mode, **kwargs)
        ...
        if self.abs_action:
            if stat['mean'].shape[-1] > 10:
                ...
                stat['min'][:3] = pcd_stat['min'][:3]
                stat['max'][:3] = pcd_stat['max'][:3]
                stat['mean'][:3] = pcd_stat['mean'][:3]
